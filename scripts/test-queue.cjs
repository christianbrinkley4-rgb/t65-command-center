...
// A DNC flag from a bulk list scrub is only a label. It never hides anyone.
const dnc = { ...base, do_not_call: true, dnc_reason: 'scrubbed' };
// A DNC somebody recorded themselves (a lead told us to stop) is the only kind that counts.
const requested = { ...base, id: 'asked', do_not_call: true, dnc_reason: 'requested' };
assert.equal(buildQueue([dnc]).length, 1, 'Scrubbed DNC flags do not remove a lead');
assert.equal(buildQueue([requested]).length, 0, 'A DNC the lead asked for removes them from the dial queue');
assert.equal(buildQueue([{ ...base, _dncSuppressed: true }]).length, 0, 'A number shared with a requested DNC is held back');
const closedDnc = withBuckets([{ ...dnc, status: 'Closed - DNC', stage_bucket: 'Closed' }])[0];
assert.equal(buildQueue([closedDnc]).length, 1, 'Scrubbed DNC status does not remove a lead');
assert.equal(buildQueue([{ ...closedDnc, _bucket: 'Closed' }]).length, 1, 'Legacy Closed bucket does not hide scrubbed DNC');
